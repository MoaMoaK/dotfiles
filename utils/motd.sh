...
function main
{
    typeset COWPATH="/usr/share/cowsay/cows/"
    typeset SEPARATOR=" || "
    typeset DEF_MSG="Welcome back"
    typeset PRIOR_SECOND=true
    typeset CENTER=true

    typeset COLS=$(tput cols)
    typeset LINES=$(tput lines)


    if [ -d $COWPATH -a -f $(command -v cowsay) -a -f $(command -v fortune) -a -f-$(command -v screenfetch) ]; then
        ...
        echo $DEF_MSG
        echo "Install cowsay, fortune-mod and screenfetch to get a nicer motd"
    fi
}
